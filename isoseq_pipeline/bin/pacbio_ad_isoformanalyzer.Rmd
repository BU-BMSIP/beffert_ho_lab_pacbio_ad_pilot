...
condition <- gsub('.*_', '', colnames(counts[-1]))
design <- data.frame(
    sampleID = sampleID,
    condition = condition
)
...
#design$sex <- factor(c('a','a','b','a','b','b'))
#design$age <- factor(c('a','a','b','a','b','b'))
```
...
switch_list <- isoformSwitchAnalysisPart2(
  ...
  pathToCPC2resultFile      = file.path(output_path, "cpc2_result.txt"),
  pathToPFAMresultFile      = file.path(output_path, "pfam_results.txt"),
  pathToIUPred2AresultFile  = file.path(output_path, "iupred2a_result.txt.gz"),
  pathToSignalPresultFile   = file.path(output_path, "signalP_results.txt"),
  outputPlots               = FALSE
)
```
