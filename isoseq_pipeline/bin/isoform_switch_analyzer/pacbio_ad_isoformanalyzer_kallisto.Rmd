...
switch_analyzer_list_df <- switch_analyzer_list_df %>%
  left_join(
    diff_gene_expr %>% select(gene_name, padj), 
    by = join_by(gene_id == gene_name)) %>%
  mutate(gene_q_value = padj) %>%
  subset(select = -padj) %>%
  left_join(
    diff_iso_expr %>% select(isoform_id, padj), 
    by = "isoform_id") %>%
...
#saveRDS(switch_analyzer_list_filtered, file = "switch_analyzer_list_filtered.rds")
```

```{r}
# significant isoform switches (iso_switch_q_value < 0.05)
switch_analyzer_list_filtered <- readRDS("./switch_analyzer_list_filtered.rds")
...
significant_iso_switches <- switch_analyzer_list_df %>%
  ...
  arrange(isoform_switch_q_value) %>%
  select(isoform_id, gene_id, condition_1, condition_2,iso_biotype, IF1, IF2, dIF, isoform_switch_q_value, gene_switch_q_value, PTC)
...
switch_analyzer_list_filtered <- analyzeDeepLoc2(
    switchAnalyzeRlist = switch_analyzer_list_filtered,
    pathToDeepLoc2resultFile = "./biolib_results/deeploc2/results_688CFAF7002A0FC1C6951370.csv",
    quiet = FALSE
)
...
ggplot(data = switch_analyzer_list_df, aes(x = dIF, y = -log10(isoform_switch_q_value))) +
  ...
  ggtitle("significant isoform switches") +
...
```
```{r}
switch_analyzer_list_filtered$switchConsequence %>% filter(gene_id == "BRD8")
switch_analyzer_list_filtered$isoformFeatures %>% filter(gene_id == "BRD8")
```
