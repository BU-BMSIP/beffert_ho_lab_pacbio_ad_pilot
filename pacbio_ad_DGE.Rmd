...
filtered_counts <- counts_by_gene[rowSums(counts_by_gene[-1] >= 10)>= 2,]
...
volcano_plot <- degs %>%
  ...
  theme_minimal() +
  labs(x = "Log2 Fold Change", y = "-Log10 padj", title = "Volcano Plot with Significant Genes Labelled")
...
```
```{r}
plotCounts(dds, "CA1", intgroup = "condition")
```
